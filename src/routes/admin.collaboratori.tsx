...
export const Route = createFileRoute("/admin/collaboratori")({ component: TeamPage });
...
function TeamPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: memSelf } = await supabase
      ...
      if (role !== "admin") {
        setLoading(false);
        // Route protetta anche via URL: reindirizza a home.
        navigate({ to: "/" });
        return;
      }
  ...
  const createInvite = async (email: string, role: AccountRole) => {
    ...
    // impedisci doppio invito attivo
  ...
  const resendInvite = async (inv: Invitation) => {
    // Rigenera il token per invalidare il precedente.
    const { data, error } = await supabase
      .from("account_invitations")
  ...
  if (loading) {
    return (
      <div>
        <TopBar title="Collaboratori" subtitle="Gestione team" />
        <div className="p-6 text-sm opacity-70">Caricamento…</div>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      {msg && (
        <div className="mx-4 md:mx-8 mt-4 px-3 py-2 rounded bg-brand-green text-brand-cream text-sm">{msg}</div>
      )}

      {!EMAIL_PROVIDER_ENABLED && (
        <div className="mx-4 md:mx-8 mt-4 px-3 py-2 rounded bg-warning/15 text-warning text-xs">
